 function [q2R,q3R,q2L,q3L]= Multiple_Trajectory_29DOF(j,showTransition,plo)
% j=4;
% showTransition=1;
% plo=1;
x=[-50 0 50]';
...
for k=0:j-1
    %FOR LEFT
            ...
            L3=322.0;
            if(rem(k,2)==0)
                basexL=[-50 -38.89 -27.78 -16.67 -5.56 5.56 16.67 27.78 38.89 50];
                basexL=basexL+K;
                baseyL=[0 8 14 18 20 20 18 14 8 0];
            else
                basexL=zeros(1,10);
                basexL=basexL+K;
                baseyL=zeros(1,10);
            ...
            q2L=zeros(1,10);
            q3L=zeros(1,10);
            h1L=[650 650 650 650 650 650 650 650 650 650];
            l1L=[-25 -19.45 -13.9 -8.35 -2.8 2.8 8.35 13.9 19.45 25];
            l1L=l1L+K;
            h1NL=h1L-baseyL;
            l1NL=l1L-basexL;
    %FOR RIGHT
            if(rem(k,2)==0)
                ...
                basexR=[-50 -38.89 -27.78 -16.67 -5.56 5.56 16.67 27.78 38.89 50];
                basexR=basexR+K;
                baseyR=[0 8 14 18 20 20 18 14 8 0];
            end
            pyR=zeros(1,10);
            pxR=zeros(1,10);
            siR=zeros(1,10);
            q2R=zeros(1,10);
            q3R=zeros(1,10);
            h1R=[650 650 650 650 650 650 650 650 650 650];
            l1R=[-25 -19.45 -13.9 -8.35 -2.8 2.8 8.35 13.9 19.45 25];
            l1R=l1R+K;
            h1NR=h1R-baseyR;
            l1NR=l1R-basexR;
    for i=1:10
